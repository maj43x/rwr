...
# GŁÓWNA NAPRAWA: Jeśli interfejs WebXR działa, sprawdzamy macierze oczu
 if webxr_interface and webxr_interface.is_initialized():
  ...
  var proj_right = webxr_interface.get_projection_for_view(1)
		...
		# Jeśli emulator jeszcze się budzi i wysyła puste/błędne dane,
		# przerywamy tę klatkę i nie pozwalamy Godotowi rzucić błędu.
  if proj_left == Projection() or proj_right == Projection():
	return

	# Tutaj zostaje Twój dotychczasowy kod od analogu:
